<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&height=300&color=gradient&text=USA%20Travel%20Guide&animation=fadeIn"/>
</p>

<p align="center">
  <a href="../../README.md">English</a> ·
  <a href="README.es.md">Español</a> ·
  <a href="README.zh.md">中文</a> ·
  <a href="README.ja.md">日本語</a>
</p>

<p align="center">
  美国旅行指南 —— 涵盖 5 大区域、12 座城市、公路路线、可筛选相册、旅行工具、季节建议、文化亮点、美国趣味冷知识，以及实用行程要点。
</p>

<p align="center">
  <a href="https://tgthms.github.io/usa-travel-guide/"><strong>🔗 在线体验</strong></a>
</p>

---

## ✨ 功能特色

| | |
|---|---|
| 🌍 **全美覆盖** | 全部 50 州，按东北、南部、中西部、西部、西南等大区组织。 |
| 🏙️ **目的地城市** | 十二座城市卡片（纯 CSS 视觉、无需配图）、评分与详情弹层。 |
| 🔗 **实用外链** | 官方旅游、公共交通、公园与博物馆链接。 |
| 🖼️ **相册** | 可筛选旅行摄影：搜索、排序、画质（缩略图 / 中等 / 原图）与灯箱。*支持 HDR* |
| 🧰 **旅行工具** | 汇率、世界时钟、小费与销售税（50 州）、油车/电车费用、紧急电话。 |
| 🌦️ **季节、文化与路线** | 何时出行、文化亮点与经典公路。 |
| 🎲 **美国趣味冷知识** | 每次随机一条、固定高度卡片；多语言数十条。 |
| 🌐 **多语言** | 英 / 西 / 中 / 日（含法律页）。 |
| ⚙️ **个性化** | 主题、语言、华氏/摄氏与英里/公里、相册画质——保存在本机。 |
| ♿️ **动画：完整 / 减弱 / 关闭** | 三级动效：完整效果、平静的轻微淡入，或关闭装饰动画。光标拖尾可单独开关。遵循系统「减少动态效果」。 |
| 📱 **移动端友好** | 轻量首屏、触控即时滚动与性能优化。 |
| 🔒 **隐私与条款** | 站内[隐私政策](../../privacy.html)与[使用条款](../../terms.html)，含外链说明。 |

---

## 🚀 快速开始

纯静态站点 — 本地打开或部署至 GitHub Pages 等。

```bash
open index.html
# 或
python3 -m http.server 8080
```

页面：`index.html` · `gallery.html` · `tools.html` · `privacy.html` · `terms.html`

---

## 💬 反馈

个人兴趣项目 —— 欢迎提交问题与建议！

📧 **contact.timg@icloud.com**

---

## 📄 版权声明

- **代码：** MIT — 见 [LICENSE](../../LICENSE)。
- **`/images` 照片：** © 2026 Tim G (TGthms)，**[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)** — 注明作者后可使用。
- **网页字体：** 来自 Google Fonts — Fraunces、Public Sans、Special Elite、Noto Sans/Serif（简中/日文）。开放许可（SIL OFL / Apache 等）。
- **站点政策：** [隐私政策](../../privacy.html) · [使用条款](../../terms.html)

建议署名：*“Photo © Tim G (@TGthms), licensed under CC BY 4.0.”*
